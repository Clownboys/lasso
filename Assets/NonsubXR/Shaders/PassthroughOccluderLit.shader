Shader "EcXR/PassthroughOccluderLit"
{
	Properties
	{
		...
		_PulsePositionAndTime ("Pulse", Vector) = (0, 0, 0, 0)
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 ShadowReceiverFragment(const Varyings input) : SV_Target {
				...
				//Global pulse effect
				half dist = distance(input.positionWS, _PulsePositionAndTime.xyz);
				half pulse = saturate(sign(_PulsePositionAndTime.w - dist) * (dist - _PulsePositionAndTime.w + 1.0f));

				alpha += pulse;
				color += half3(1, 1, 0) * pulse;
